...
class FeatureEngine:
    ...
    def compute_distress_features(inventory_df: pl.DataFrame) -> pl.DataFrame:
        ...
        df = df.with_columns([
            (pl.col("days_since_last_sale") / 60.0).clip(0, 1).alias("non_moving_score")
        ])
        ...
        df = df.with_columns([
            ...
            ).clip(0.01, None).alias("avg_daily_units_30d")
        ])

        # 3. Days of cover
        df = df.with_columns([
            (pl.col("on_hand_qty") / pl.col("avg_daily_units_30d")).alias("days_of_cover")
        ])
        ...
        df = df.with_columns([
            (pl.col("inventory_age_days") / pl.col("shelf_life_days")).clip(0, 1).alias("decay_score")
        ])

        # 7. Storage burden score
        df = df.with_columns([
            (pl.col("package_volume_cm3") * pl.col("on_hand_qty")).alias("storage_burden")
        ])
        ...
        df = df.with_columns([
            ((pl.col("list_price") - pl.col("unit_cost")) / pl.col("list_price")).alias("gross_margin_pct")
        ])

        # 10. Margin buffer score
        df = df.with_columns([
            (pl.col("gross_margin_pct") / 0.60).clip(0, 1).alias("margin_buffer_score")
        ])

        # 11. Overall Distress Score (weighted combination)
        df = df.with_columns([
            (
    ...
    def compute_with_custom_weights(
        ...
        df = df.with_columns([
            (
                ...
                weights.get("demand_weakness", 0.10) * pl.col("demand_weakness_score")
            ).alias("distress_score")
        ])
